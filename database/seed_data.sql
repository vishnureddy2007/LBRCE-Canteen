...
-- Add a single placeholder image for each food item
INSERT INTO food_images (food_item_id, image_url, is_primary)
SELECT f.id, CONCAT('/uploads/placeholder-', LOWER(REPLACE(f.name, ' ', '-')), '.jpg'), TRUE
FROM food_items f
WHERE NOT EXISTS (
    SELECT 1 FROM food_images fi WHERE fi.food_item_id = f.id
);
